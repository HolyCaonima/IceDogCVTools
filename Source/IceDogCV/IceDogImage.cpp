...
Pixel* IceDogImage::GetPixelDataCopy()
{
	// malloc the memory
	...
	// mem copy
	memcpy(targetDataUcharPtr, reinterpret_cast<uchar*>(imgData.get()), sizeof(uchar) * 3 * width*height);
...
std::vector<Pixel> IceDogImage::FindClusterSeed(int numCluster)
{
	...
	while (numCluster != 0)
	{
		...
		int maxDisIndex = 0;
		for (size_t i = 1; i < distanceVec.size(); i++)
		{
...
IceDogImage IceDogImage::GenerateImageUseCustomKernel(Kernel kl)
{
	if (colorSpace == ColorSpace::HSV)
	{
		ConvertToBGR();
		auto resultttt = GenerateImageUseCustomKernel(kl);
		ConvertToHSV();
		return resultttt;
	}
	auto procKernel = kl;
	int kernelSize = kl.GetKernelSize();
...
std::vector<IceDogImage> IceDogImage::SplitImageUsingKeamsPP(int numCluster)
{
	...
	while (true)
	{
		...
		{
			int minDistance = 0;
			for (size_t j = 1; j < clusterCenter.size(); j++)
			{
	...
	std::vector<IceDogImage> resultImg(numCluster);
